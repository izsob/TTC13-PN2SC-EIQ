...
import pn2sc.queries.util.TransitionQuerySpecification;

/**
 * Generated pattern matcher API of the pn2sc.queries.transition pattern, 
...
public class TransitionMatcher extends BaseMatcher<TransitionMatch> {
  ...
  protected TransitionMatch tupleToMatch(final Tuple t) {
    try {
    	return new TransitionMatch.Immutable((PetriNet.Transition) t.get(POSITION_T));	
  ...
  protected TransitionMatch arrayToMatch(final Object[] match) {
    try {
    	return new TransitionMatch.Immutable((PetriNet.Transition) match[POSITION_T]);
  ...
  protected TransitionMatch arrayToMatchMutable(final Object[] match) {
    try {
    	return new TransitionMatch.Mutable((PetriNet.Transition) match[POSITION_T]);
    } catch(ClassCastException e) {engine.getLogger().error("Element(s) in array not properly typed!",e);	//throw new IncQueryRuntimeException(e.getMessage());
    	return null;
    }
