...
import statecharts.diagram.edit.commands.StateNextCreateCommand;
import statecharts.diagram.edit.commands.StateNextReorientCommand;
...
public class AND2ItemSemanticEditPolicy extends
	...
	public AND2ItemSemanticEditPolicy() {
		super(StateChartsElementTypes.AND_3001);
	}
	...
	protected Command getDestroyElementCommand(DestroyElementRequest req) {
		...
		for (Iterator<?> it = view.getTargetEdges().iterator(); it.hasNext();) {
			...
			if (StateChartsVisualIDRegistry.getVisualID(incomingLink) == StateNextEditPart.VISUAL_ID) {
		...
		for (Iterator<?> it = view.getSourceEdges().iterator(); it.hasNext();) {
			...
			if (StateChartsVisualIDRegistry.getVisualID(outgoingLink) == StateNextEditPart.VISUAL_ID) {
				DestroyReferenceRequest r = new DestroyReferenceRequest(
	...
	private void addDestroyChildNodesCommand(ICompositeCommand cmd) {
		...
		for (Iterator<?> nit = view.getChildren().iterator(); nit.hasNext();) {
			...
			switch (StateChartsVisualIDRegistry.getVisualID(node)) {
			case ANDANDContainsCompartment2EditPart.VISUAL_ID:
				for (Iterator<?> cit = node.getChildren().iterator(); cit
						.hasNext();) {
					Node cnode = (Node) cit.next();
					switch (StateChartsVisualIDRegistry.getVisualID(cnode)) {
					case AND2EditPart.VISUAL_ID:
						for (Iterator<?> it = cnode.getTargetEdges().iterator(); it
								.hasNext();) {
							Edge incomingLink = (Edge) it.next();
							if (StateChartsVisualIDRegistry
	...
	protected Command getStartCreateRelationshipCommand(
		...
		if (StateChartsElementTypes.StateNext_4001 == req.getElementType()) {
